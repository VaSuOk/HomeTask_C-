...
namespace StructConlsole
{
    struct Result
    {
        ...
        // Написав один конструктор, який замінює два простих ( конструктор по замовчуванню)
        // Для мене він зручніший :)
    ...
    class Program
    {
        ...
        {
            // консоль не читає букву і
            Student[] MyStudents;
            MyStudents = ReadStudentsArray(); // ініціалізація масива
            int BestAveragePoints = 0, WorstAveragePoints = 0;

            PrintStudents(MyStudents); //Вивід всіх студентів

            GetStudentsInfo(MyStudents, out BestAveragePoints, out WorstAveragePoints); // визначення найкращого і найгіршого середніх балів.
            Console.WriteLine("Найкращий середнiй бал - " + BestAveragePoints + ("Найгiрший середнiй бал - " + WorstAveragePoints + (" (З усiх студентiв) ")));

            MyStudents = SortStudentsByPoints(MyStudents); // Сортування по середніх балах.
            Console.WriteLine("");
            Console.WriteLine("Вивiд списку студентiв пiсля сортування по середньому балу");
            Console.WriteLine("");
            PrintStudents(MyStudents); //Вивід всіх студентів

            MyStudents = SortStudentsByName(MyStudents);// Сортування по ініціалах.
            Console.WriteLine("");
        ...
        static Student[] ReadStudentsArray()
        {
            ...
            for (int i = 0; i < size_student; i++)
            {
                ...
                Console.WriteLine(""); // замість endline 
